...
class RakeDbService:
    ...
    def get_train_details(query_values,rake_id=None,track_number=None,from_date=None,to_date=None,count=Constants.KEY_RETRY_COUNT,isRetry=Constants.KEY_RETRY_VALUE):
        try:
            ...
            if isRetry and count >= 0 :
                count=count-1 
                time.sleep(Constants.KEY_RETRY_TIMEDELAY) 
                RakeDbService.get_train_details(query_values,rake_id,track_number,from_date,to_date,count,isRetry)
    ...
    def get_rake_plan(rake_id,train_number=None,track_number=None,count=Constants.KEY_RETRY_COUNT,isRetry=Constants.KEY_RETRY_VALUE):
        try:
            ...
            data = rake_query.all()
            return RakeDbService.format_rake_plan(data)
        except Exception as e:
            logger.exception(str(e))
            if isRetry and count >= 0 :
                count=count-1 
                time.sleep(Constants.KEY_RETRY_TIMEDELAY)
                RakeDbService.get_rake_plan(rake_id,train_number,track_number,count)
