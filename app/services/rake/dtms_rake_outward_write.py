...
class DTMSRakeOutwardWriteService:

    # TODO : Need to update this function once Soap API is integrated
    def format_data_to_ccls_format(data):
        rake_data = {}
        rake_data["trainNumber"] = "TEST"
        rake_data["INOUT_FLAG"] = "O"
    ...
    def update_container(data,count=Constants.KEY_RETRY_COUNT,isRetry=Constants.KEY_RETRY_VALUE):
        try:
            if config.GROUND_TRUTH == GroundTruthType.ORACLE.value:
                ...
                ccls_data = DTMSRakeOutwardWriteService.format_data_to_ccls_format(data)
                result = soap_service.update_domestic_outward_rake(ccls_data,Constants.UPDATE_RAKE_CONTAINER_ENDPOINT)
                return result
            return {}
            ...
            if isRetry and count >= 0 :
                ...
                DTMSRakeOutwardWriteService.update_container(data,count)

    @query_debugger()
    def update_VGO_survey(data,count=Constants.KEY_RETRY_COUNT,isRetry=Constants.KEY_RETRY_VALUE):
        try:
            if config.GROUND_TRUTH == GroundTruthType.ORACLE.value:
                pass
            elif config.GROUND_TRUTH == GroundTruthType.SOAP.value:
                ccls_data = DTMSRakeOutwardWriteService.format_data_to_ccls_format(data)
                result = soap_service.update_domestic_outward_rake(ccls_data,Constants.CGI_SURVEY_ENDPOINT)
                return result
            return {}
        except Exception as e:
            logger.exception(str(e))
            if isRetry and count >= 0 :
                count=count-1 
                time.sleep(Constants.KEY_RETRY_TIMEDELAY)
                DTMSRakeOutwardWriteService.update_VGO_survey(data,count)
